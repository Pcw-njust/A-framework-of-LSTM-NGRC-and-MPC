...
# 1. 数据生成
# ---------------------------------------------------------
def lotka_volterra(z, t, R, beta, alpha, lamda):
    x, y = z
    return [0.5 * x * y - 0.5 * (x**2) / 1.0 - beta * x / (x + 1), R - alpha * y - lamda * x * y]

params = (0.1, 0.6, 0.45, 0.15)
z0 = [0.6689, 0.9085]
dt = 0.001
t_total = np.arange(0, 50, dt)
sol = odeint(lotka_volterra, z0, t_total, args=params)

split = int(len(sol) * 0.06)
...
plt.plot(t_train, train_data[:, 0], 'k-', label='True x', alpha=0.8)
plt.plot(t_train_plot, train_predictions[:, 1], 'g:', label='NGRC y (训练集)', alpha=0.8)
plt.plot(t_train, train_data[:, 1], 'b-', label='True y', alpha=0.8)
