...
# 1. 系统定义
# ---------------------------------------------------------
def custom_system(z, t, R, beta, alpha, lamda, u_x=0, u_y=0):
    x, y = z
    dxdt = 0.5 * x * y - 0.5 * (x**2) / 1.0 - beta * x / (x + 1) + 0.5*u_x
    dydt = R - alpha * y - lamda * x * y + 0.95*u_y
    return [dxdt, dydt]
...
sol_evolve = odeint(custom_system, sol_hold[-1], t_evolve, args=(0.1, 0.6, 0.45, 0.15, 0, 0))
...
def mpc_cost(u_seq, window_in, target, model, poly_transformer, h, delta_u_penalty, u_prev, dt):
    ...
    for i in range(h):
        ...
        def dynamics_approx(z, t):
            # NGRC 预测的无控导数 + 控制量
            return dXdt_pred + np.array([0.5*u_x_step, 0.95*u_y_step])

        # 3. 使用 odeint 积分得到下一时刻状态
        # 积分区间：0 到 dt
        next_s = odeint(dynamics_approx, current_s, [0, dt])[-1]

        # 计算代价
        cost += np.sum((next_s - target) ** 2) * 50
...
for t in t_ctrl:
    ...
    next_actual = odeint(
        ...
        actual_now,
        [0, dt],
        args=(0.1, 0.6, 0.45, 0.15, u_x_opt, u_y_opt)
    )[-1]
